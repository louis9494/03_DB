...
-- V_DCOPY2 VIEW를 이용해서 INSERT 수행 (원래는 안됨)
INSERT INTO V_DCOPY2 
VALUES('D0','L2');
--> 가상 테이블 VIEW 인데 INSERT 가 성공!

-- V_DCOPY2 VIEW  에서 INSERT 한 결과 확인
SELECT * FROM V_DCOPY2;
-- DO L2까지
...
-- 반대로 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL 이 들어감!
	...
	-- 가능하면 의도되지 않은 NULL 은 존재하지 않게 해줘야함
...
-- VIEW 를 통해서 DML은 하지 않아야 한다!!

/* WITH READ ONLY 옵션 사용하기*/
-- WHY?
--> VIEIW를 이용해서 DML(INSERT/UPDATE/DELETE) 하지 않기 위해서!
 ...
 *            == 마지막으로 호출한 NEXTVAL 값을 반환, 바로 생성 못함
 * */
...
--> CURRVAL의 정확한 의미 : 가장 최근 호출된 NEXTVAL의 값을 반환함
--> NEXTVAL ㅡ를 호출핝거이 없어서 오류 발생!!

--> 해결방법 : NEXTVAL 호출하면 해결된다.
...
WHERE TEST_NAME = '짱구';
--> 짱구의 TEST_NO 값을 150 rKWL WMDRKTLZLRHSKTJ
 ...
 *   컬럼에 대하여 생성하는 객체
 *
 * - 인덱스 내부 구조는 B* 트리(B-star tree(rute)) 형식으로 되어있음.
 *
 *  
 *
 * ** INDEX의 장점 **
...
-- TB_IDX_TEST 테이블에
-- 샘플 데이터 100만개 삽입 (PL/SQL (논리적흐름의 제거 기능이 가능) 사용)
-- 조건문(IF/CASE),반복문(LOOP, WHILE) 사용 가능
BEGIN 
	FOR I IN 1..1000000 -- I 변수 (IN) 안에 1부터 100만까지 반복
	LOOP
