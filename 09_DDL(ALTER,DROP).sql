...
-- 객체를 만들고(CREATE), 바꾸고(ALTER), 삭제(DROP) 하는 데이터 정의언어

/*
 * ALTER(바꾸다, 수정하다, 변조하다)
 *
 * -- 테이블에서 수정할 수 있는 것
 * 1) 제약 조건(새로운 것을 추가/ 기존의 것을 삭제)
 * 2) 컬럼(추가/ 수정/ 기존의 것 삭제)
 * 3) 이름변경 (테이블명, 컬럼명..)
 *
 *
...
-- [작성법]
-- 1) 추가 : ALTER TABLE 테이블명 -- 수정
--          ADD [CONSTRAINT 제약조건(UNIQUE, PRIMARY KEY..)명] 제약조건 추가(지정할컬럼명)
--          [REFERENCES 테이블명[(컬럼명)]]; <-- FK 인 경우 추가(외래키)




-- 2) 삭제 : ALTER TABLE 테이블명 DROP(삭제) CONSTRAINT 제약조건명;


-- * 제약조건 자체를 수정하는 구문은 별도 존재하지 않음!
--> 삭제 후 추가를 해야함.

-- DEPARTMENT 테이블 복사본 복사(컬럼명, 데이터타입, NOT NULL 만 복사)
CREATE TABLE DEPT_COPY 
AS SELECT * FROM DEPARTMENT; -- 모든 데이터 복사
...
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NOT NULL; --DEPT_TITLE 컬럼에 NULL(값) 비허용

ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NULL; -- DEPT_TITLE 컬럼에 NULL(값) 허용
...
SELECT * FROM DEPT_COPY;
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입되었음을 확인했다.
...
SELECT * FROM DEPT_COPY; -- 오류(테이블명 변경)
...
--> 해결방법
-- 1) 자식, 부모테이블 순서로 삭제 
-- 2) ALTER 를 이용해서 FK 제약조건 삭제
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용 (많이씀)
	--> CASCADE CONSTRAINT : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제
...
/* DDL 주의 사항 */
-- 1) DDL은 COMMIT/ROLLBACK 이 되지 않음. 바로 반영됨!

-- 2) DDL과 DML 구문 섞어서 수행하면 안된다!
-- DDL(CREATE, ALTER, DROP : DB객체를 생성/수정/삭제)
...
ROLLBACK;
SELECT * FROM TB2; -- 롤백 안된다.. DDL구문중 ALTER를 사용해서 그 시점에 COMMIT 됨