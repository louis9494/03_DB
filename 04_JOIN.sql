...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 내부조인
...
-- (JOIN이 안되는 행도 결과 포함) == NULL 포함

-- ANSI 기준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
LEFT /*OUTER*/ JOIN DEPARTMENT
ON (DEPT_ID = DEPT_CODE); -- 23행 (DEPT_CODE 가 NULL 포함 -> 하동운, 이오리 )
...
-- 테이블마다 별칭 작성(미작성시 열의 정의가 애매하다 오류발생)
