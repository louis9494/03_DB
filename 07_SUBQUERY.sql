 ...
 * -- 메인쿼리 (==외부 쿼리, 기존쿼리)를 위해 보조 역활을 하는 쿼리문
 * 
 * - 메인쿼리가 SELECT 문일 때
...
WHERE EMP_NAME = '노옹철'-- 1380000 서브쿼리
...
SELECT EMP_NAME, JOB_NAME, DEPT_TITLE ,DEPT_CODE, SALARY
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 2) 부서별 급여합이 17,700,000DLS 부서의 부서명과 급여합 조회
...
-- 부서별 최고 급여를 받는 직원의 이름, 직급, 부서, 급여를
-- 부서 순으로 정렬하여 조회
...
-- 2)  DEPARTMENT 테이블의 위의 결과(L1)와 동일한LOCATION_ID를 가지고 있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
SELECT EMP_ID, EMP_NAME , DEPT_CODE, HIRE_DATE , DEPT_TITLE, JOB_NAME
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
...
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGER_ID가 실제로 직원
-- 테이블의 EMP_ID와 일치하는 것을 확인
...
-- 상관쿼리 (이태림이 D8부서에서 가장 빠른입사&퇴사자여서 걸러짐 - D8 부서 아에 제외)
SELECT EMP_ID , EMP_NAME ,DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE  
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID  = DEPT_CODE)
...
WHERE SUB.DEPT_CODE = MAIN.DEPT_CODE
AND ENT_YN = 'N' 
OR (SUB.DEPT_CODE IS NULL AND MAIN.DEPT_CODE IS NULL)) -- 소속없음까지 포함
ORDER BY HIRE_DATE;
...
-- 즉, SELECT 절에 작성되는 단일행 서브쿼리를 스칼라 서브쿼리라고 함.
...
--> SELECT문의 해석 순서 때문에 급여 상위 5명이 아니라
-- 조회 순서 상위 5명의 급여 순위 조회됨
...
SELECT ROWNUM, DEPT_CODE, DEPT_TITLE, 평균급여 
	FROM (SELECT DEPT_CODE, DEPT TITLE, CEIL(AVG(SALARY)) 평균급여
	FROM EMPLOYEE 
	LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
...
-- 9. RANK() OVER / DENSE_RANK() OVER -- 서브쿼리랑 관련이 있다고 하기는 애매함
...
--RANK () OVER (정렬순서) / DENSE_RANK() OBER(정렬순서)
...
SELECT EMP_ID, EMP_NAME, PHONE, SALARY, JOB_NAME
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE) -- ON을 실무에서 자주 쓰임
WHERE SALARY = (SELECT MAX(SALARY) 
				FROM EMPLOYEE
         		WHERE HIRE_DATE >= TO_DATE('2001-01-01','YYYY-MM-DD')); 
         		-- TO_DATE를 연도만 쓰면 10월 이후로 컴퓨터가 출력함
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--NATURAL JOIN JOB
JOIN JOB USING(JOB_CODE) --선생님 풀이
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
							   FROM EMPLOYEE
                               WHERE EMP_NAME = '노옹철')
...
FROM EMPLOYEE MAIN 
JOIN JOB SUB1 ON(MAIN.JOB_CODE = SUB1.JOB_CODE)
...
SELECT FLOOR(MONTHS_BETWEEN(SYSDATE , TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) / 12), 
