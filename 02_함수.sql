...
-- EMPLOYEE 테이블에서 사번이 홀수사원의 사번, 이름 조회
...
SELECT ROUND(123.456) FROM DUAL; -- 123, 수점 첫 번째 자리에서 반올림
...
SELECT ABS(ROUND(MONTHS_BETWEEN (SYSDATE, '2025-01-23'), 3)) "수강 기간"
FROM DUAL; 
...
SELECT TO_DATE('800505', 'RRMMDD') FROM DUAL;
...
FROM EMPLOYEE;
--2) 추출한 생년월일을 DATE 타입으로 변경 -> RR 패턴을 이용하여 1900 년도로 바꿈
SELECT EMP_NAME,
...
CASE WHEN SALARY >= 5000000 THEN '대' -- LIKE IF문
