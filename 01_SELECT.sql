/*
 * SLELECT (DML 또는 DQL) : 조회
 * 
 * - 데이터를 조회SELECT)하면 조건에 맞는 행들이 조회됨.
 * 이 때, 조회된 행들의 집합을 "RESULT SET" 이라고 한다.
 * 
 * - RESULT SET은 0개 이상의 행을 포함할 수 있다.
 * 왜 0개? 조건에 맞는 행이 없을 수도 있음.
 * 
 * 
 * */
...
-- * : ALL, 모든, 전부
--> EMPLOYEE 테이블에 colunm에 모든 colunm을 조회하겠다.
 ...
 * 
 * 컬럼명 AS "별칭" : 별칭 내에 띄어쓰기 O, 특수문자 O, 문자만 O
 * 
 * AS 생략 가능 
 * 
 * 
...
-- DISTINCT : 조회 시 컬럼에 포함된 중복 값을 한번만 표기 -> 중복 제거할 때 쓰는 키워드
...
---3. SELECT 절(조회) : SELECT 컬럼명
-- 1. FROM 절 : FROM 테이블명 -> 이 테이블에서 조회하겠다
-- 2. WHERE 절(조건절) : WHERE 컬럼명 연산자 값;
-- 4.ORDER BY 컬럼명 | 별칭 | 컬럼 순서 | [ASC | DESC] [NULLS FIRST | LAST]
-- 1 > 2 > 3 > 4 해석순서

-- EMPLOYEE 테이블에서 (FROM절) 
-- 급여가 3백만원 초과인 사원(WHERE 절)의
-- 사번, 이름, 급여, 부서코드를 (SELECT절)조회해라
...
SELECT EMP_ID, EMP_NAME, EMAIL, DEPT_CODE, HIRE_DATE, DEPT_CODE, SALARY
FROM EMPLOYEE
...
-- EMPOLOYEE 테이블에서 보너스가 있는 사원의 이름, 보너스 조회
-- SELECT * FROM EMPLOYEE;
SELECT  EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL;

-- EMPOLOYEE 테이블에서 보너스가 없는 사원의 이름, 보너스 조회
-- SELECT * FROM EMPLOYEE;
SELECT  EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL;
